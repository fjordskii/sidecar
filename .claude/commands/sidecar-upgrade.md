...
- **`files.system_if_uninitialized`** (`LOOP_PROMPT.md`, `JOURNAL.md`, `ops/run.sh`) — replace
...
4. **Apply the update** if upstream is strictly newer (semver): fetch every `files.system` path,
   fetch `files.system_if_uninitialized` paths that still pass the token grep, and copy upstream
   `VERSION`. If local and upstream versions match, say so and stop after step 3.
