...
- **Reflect what you heard**, in their words, after each section. Wrong answers captured early get
  expensive.
...
- **Want per-cycle commentary on accounts this loop can't trade?** The reference setup ends every
  cycle with a one-line read on separate retirement accounts, advice only. Costs one line and is
  often useful. If yes: which accounts, which holdings, what decisions they'd want flagged.
...
- **What do you actually believe about markets?** Being right about a story others haven't priced?
  Buying quality and waiting? Catching mispricings? Riding momentum? There's no correct answer, but
  the loop needs *one*. An agent with no stated edge reverts to buying whatever was in the news that
  morning.
...
- **Concentration ceiling**: the most any single name may be. Trimming back past it is how a good
  call is kept from becoming the whole book.
...
- **How much autonomy?** The consequential setting:
  ...
  - **Propose-only**: full analysis and journaling, orders wait for a human. A good way to
...
- **Where does it run?** Cloud or local. Capture the preference; `SETUP.md` covers both.
...
- **`ops/run.sh`**: fill `{{REPO_PATH}}`, `{{ACCOUNT_ID}}`, `{{BROKER}}`, `{{MCP_SERVER}}`,
  ...
  local job fails without an error. If they chose cloud-only, still fill it; it's their fallback.
...
If they chose a cloud routine, do **not** point at `ops/ROUTINE_PROMPT.md` and leave them to
...
missing `Bash` tool or a locally added but unconnected MCP server produces a routine that runs,
